...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        ...
        {
            arr_small[count2++] = arr_big[i];	//将arr_big[i]统计字母出现的次数记录在arr_small里/*补充注释（说明作用）*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //分左右结点/*补充判断模块注释*/
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    {
    	if(huffman[k][3]==-1&&huffman[k][0]==-1)
    	{
    		break;
		}
	  if(huffman[k][3]<min&&huffman[k][0]==-1&&k!=ii[0])
	  {
	  	min1=huffman[k][3];
	  	ii[1]=k;
	  }
...
{
    int length = 0;
    if(huffman[i][0] == -1)   //父结点不存在  /*补充判断模块注释*/
    {
        return 0;
    }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    while(father != -1)
    {
        if(huffman[father][1]==current){/*自行填充代码*/		//判断当前结点的父结点左子树是否为当前结点
        code[start]=='0';}/*自行填充代码*/		//子结点是父结点的左子树，编码为0
        else{code[start]=='1';}/*自行填充代码*/		//子结点是父结点的右子树，编码为0

        current=father;/*自行填充代码*/		//往上朔源，更新当前结点
        father=huffman[father][0];/*自行填充代码*/		//同理（当前结点更新后），更新当前结点的父亲结点
        start++;/*自行填充代码*/		//更新编码位置
    }
    code[start]='\0';   //编码结束符
