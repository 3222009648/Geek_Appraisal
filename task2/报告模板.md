...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        ...
        {
            arr_small[count2++] = arr_big[i];	/*将arr_big[i]统计字母出现的次数放入arr_samll中*/
        }
    }
    ...
    { 
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //初始化左右节点///*补充注释*/
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        ...
        if(huffman[i][3] < min && huffman[i][0] == -1)  //找出权重最小的一个节点///*补充判断模块注释（说明作用）*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for (int k = 0; k < 2 * n - 1; k++)
    {
	    ...
	    if(huffman[k][3] < min1 && huffman[k][0] == -1 && k!= ii[0])
	    {
	    	min1 = huffman[k][3];
	    	ii[1] = k;
		}
...
{
    int length = 0;
    if(huffman[i][0] == -1)     //如果父节点不存在///*补充判断模块注释*/
    {
        return 0;
    }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    while(father != -1)
    {
		...
		{
		    code[start] = '1';/*自行填充代码*/		//子结点是父结点的右子树，编码为1
        }
...
![image-20221105205606567](C:\Users\21970\AppData\Roaming\Typora\typora-user-images\image-20221105205606567.png)
...
1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？答：每生成两个叶子原来一个叶子就变成了结点。
2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？答：遍历大小写字母
